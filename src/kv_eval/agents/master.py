...
from ..reporting.sections import validate_report, citeable_evidence, PAPER_CITATION as PAPER_CITATION_RE, WEB_CITATION as WEB_CITATION_RE
...
def cited_snippets(state: dict) -> list[dict]:
    """검수 Judge에게 보고서가 실제 인용한 근거만 원문과 함께 넘긴다.

    전체 카탈로그를 길이 제한으로 자르면 뒤쪽 인용([W…], 뒷페이지)이 잘려 나가
    "제공된 근거에 없는 인용"이라는 오판이 반복된다. 인용된 항목만 넘기면 모두 들어간다.
    """
    body = state["report_draft"].split("\n## REFERENCE", 1)[0]
    used = set(PAPER_CITATION_RE.findall(body)) | {f"W{n}" for n in WEB_CITATION_RE.findall(body)}
    out = []
    for ev in citeable_evidence(state["evidence"]):
        cite = ev["citation"]
        key = cite.strip("[]")
...
def master_report_gate_node(state: dict, llm: Any) -> dict:
    ...
    judged = llm.generate_structured(
        ...
        "Items marked [D] come from the team's own design document (listed in REFERENCE as an internal document): "
        "they are the stated design premises/assumptions, not research findings, so do not require paper/web evidence for them; "
        "instead check that the KV cache table arithmetic matches its stated formula. "
        "Signal tables in 4.1-4.4 and the evidence balance table are generated by code from the agents' verdicts; "
        "flag only contradictions between them and the narrative.\n"
